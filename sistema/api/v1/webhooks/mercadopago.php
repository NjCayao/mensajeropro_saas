    ...
    if ($subscription['status'] === 'authorized') {
        ...
        try {
            ...
            if ($suscripcion_existente) {
                ...
                ");
                $stmt->execute([
                    $pago['plan_id'], $tipo, $fecha_fin, $fecha_proximo,
                    $monto, $preapproval_id, $preapproval_id, 
                    json_encode($subscription), $suscripcion_existente['id']
                ]);
            } else {
                // Crear nueva
                $stmt = $pdo->prepare("
                ...
                ");
                $stmt->execute([
                    $pago['empresa_id'], $pago['plan_id'], $tipo, $fecha_fin,
                    $preapproval_id, $preapproval_id, $fecha_proximo, $monto,
