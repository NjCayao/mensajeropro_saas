...
// Leer configuración desde el archivo PHP
function getDBConfig() {
  try {
    const configPath = path.join(__dirname, "../../config/database.php");
    const configContent = fs.readFileSync(configPath, "utf8");

    // Extraer las constantes del archivo PHP
    const config = {};
    const regex = /define\(['"](\w+)['"]\s*,\s*['"]([^'"]+)['"]\)/g;
    let match;
    ...
    return {
      ...
      database: config.DB_NAME || "mensajeropro_saas",
    };
  } catch (error) {
    console.error("Error leyendo config PHP, usando valores por defecto");
    // Valores por defecto si falla
    return {
      host: process.env.DB_HOST || "localhost",
      user: process.env.DB_USER || "root",
      password: process.env.DB_PASS || "",
      database: process.env.DB_NAME || "mensajeropro_saas",
    };
  }
}
...
async function actualizarEstadoMensajeProgramado(
  id,
  estado,
