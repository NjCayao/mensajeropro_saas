...
try {
    ...
    if ($tipo_envio === 'individual') {
        ...
        $stmt = $pdo->prepare("SELECT nombre FROM contactos WHERE id = ? AND activo = 1");
        ...
        $stmt = $pdo->query("SELECT COUNT(*) FROM contactos WHERE activo = 1");
        ...
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM contactos WHERE categoria_id = ? AND activo = 1");
        $stmt->execute([$categoria_id]);
        $total_destinatarios = $stmt->fetchColumn();
    }
    ...
    $stmt = $pdo->prepare("
        ...
        (usuario_id, titulo, mensaje, imagen_path, categoria_id, enviar_a_todos, 
         fecha_programada, estado, total_destinatarios)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'pendiente', ?)
    ");
    ...
    // Log
    logActivity($pdo, 'mensajes', 'programar', 
